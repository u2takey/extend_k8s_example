...
type NginxAppLister interface {
	...
	List(selector labels.Selector) (ret []*simple.NginxApp, err error)
...
// List lists all NginxApps in the indexer.
func (s *nginxAppLister) List(selector labels.Selector) (ret []*simple.NginxApp, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*simple.NginxApp))
	})
	return ret, err
}
...
type NginxAppNamespaceLister interface {
	...
	Get(name string) (*simple.NginxApp, error)
...
// List lists all NginxApps in the indexer for a given namespace.
func (s nginxAppNamespaceLister) List(selector labels.Selector) (ret []*simple.NginxApp, err error) {
...
// Get retrieves the NginxApp from the indexer for a given namespace and name.
func (s nginxAppNamespaceLister) Get(name string) (*simple.NginxApp, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(simple.Resource("nginxapp"), name)
	}
	return obj.(*simple.NginxApp), nil
}
